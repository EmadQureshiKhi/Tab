 ...
 *
 * ## This is a deliberate re-expression of `apps/gateway/src/witness.ts`
 *
 * The dependency graph is one-way and linted: `services/*` may depend on
 * `packages/*` and on nothing else, so the gateway's builder cannot be imported
 * from here however much this package would like to. The alternative to
 * re-expressing it would be widening the dependency rule, which is a worse trade
 * than one duplicated module. The fold, the ABIs, the chunking constants and the
 * ordering rule below are therefore the gateway's, kept identical on purpose, and
 * the report for this task names the promotion into `packages/sdk` that would
 * retire the copy.
 *
 * ## Why a witness at all
 *
 * `TabBook.recordDelivery` will not charge an Agent without one. The witness is the
 * Agent's full ordered Verified Settlement history plus one Bond entry per
 ...
 * bound, which is what stops a caller flipping `curated` or `bonded` to manufacture
 * credit through the very commitment meant to prevent it.
 *
 ...
 * still worth doing, because it lets a caller compute the same Credit Limit locally
 * and notice a disagreement before spending gas, but it is never trusted.
 *
 * Requirements: 22.3, 22.4, 13.1, 13.2, 17.2, 17.3
 */
 ...
 *
 * `TabBook.recordDelivery` opens with `_requireOperator`, comparing `msg.sender`
 ...
 * registration: this Service's software either holds that key or it can record no
 * delivery at all.
 *
 * The trap is that a distinct key per process looks like good hygiene and is wrong
 * here. The gateway and this Service are two processes of one Service, and one Service
 * has one operator. Neither cheap check notices: a keyless simulation passes, because
 * `eth_call` is made with the operator address as `from`, and the broadcast reverts
 * `NotServiceOperator` only after gas has been spent.
 *
 * The same check lives in `apps/gateway/src/witness.ts`. It is repeated rather than
 * imported because `services/*` may depend on `packages/*` only, which is the same
 * reason this whole module is a re-expression rather than a shared one.
 */
