 ...
 *
 * ## A deliberate re-expression of `apps/gateway/src/tab-book.ts`
 *
 * The dependency rule allows `services/*` to reach `packages/*` alone, so the
 * gateway's client is not importable here. Every measured fact it records is a
 * fact about the same deployed contract this service meters against, so the
 * module is kept identical rather than paraphrased, and the report for this task
 * names the promotion into `packages/sdk` that would retire both copies.
 *
 ...
 *
 * Requirements: 22.3, 22.5, 12.1, 12.2, 12.3, 23.3
 */
...
function refusal(error: unknown, what: string): TabError {
  ...
  if (decoded !== undefined) {
    return {
      category: decoded.name === "LimitExceeded" ? "LIMIT" : "CHAIN",
      code: decoded.name,
      message: `${what} was refused: ${decoded.name}(${Object.entries(decoded.args)
        .map(([name, value]) => `${name}=${value}`)
        .join(", ")})`,
      retryable: false,
      details: { revert: decoded.name, ...decoded.args },
    };
  }
  return {
    category: "CHAIN",
...
export interface ProofServiceTabBookClient {
 ...
 */
export function toSdkTabBookClient(client: ProofServiceTabBookClient): {
...
export function createTabBookClient(options: TabBookClientOptions): ProofServiceTabBookClient {
